...
The [xdode.exchange](http://xdode.exchange) Marketplace is an online business directory where competing suppliers (freelancers and business) can create profiles and bid for tenders and proposals from companies seeking Professional Services and products.
This allows companies and to source the right products and services, at the right price, at the right time and at the right quality.

## Functions of [xdode.exchange](http://xdode.exchange)

This project was bootstrapped with [Create React App](https://github.com/facebook/create-react-app).

The functions of the application will include: 
A services directory for organisations seeking to outsource specialised skills from freelancers and businesses. Clients can either submit tender proposals (or detailed briefs for projects) for businesses to apply and bid for or directly approach a business through a “yellow-pages” type directory. The app will allow organisations looking for Professional Services and products to compare various service offerings from competing companies and freelancers using attributes like price, ratings etc.  
...
-	The application will then facilitate the payment for the services. 
-	POS for open markets , allowing users to pay via QR code 
-	Project Management Off the project – integration with existing systems is crucial here 
...
You can see the full change in [routes/api/profile.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/4be414c6a54994c18397dba9c927ad67b878508b/routes/api/profile.js#L324)

## uuid no longer has a default export

The npm package [uuid](https://www.npmjs.com/package/uuid) no longer has a default export, so in our [client/src/actions/alert.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/actions/alert.js) we need to change the import and use of this package.
...
You can see the use here in [routes/api/profile.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/31e5318592b886add58923c751dba73274c711de/routes/api/profile.js#L71)
...
You can see the code use here in [routes/api/users.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/routes/api/users.js#L44)
...
The subscription can be seen in [client/src/store.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/store.js)

We also need to change our [client/src/utils/setAuthToken.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/utils/setAuthToken.js) so it now handles both the setting of the token in local storage and in axios headers.

With those two changes in place we can remove all setting of local storage from [client/src/reducers/auth.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/reducers/auth.js). And remove setting of the token in axios headers from [client/src/actions/auth.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/actions/auth.js). This helps keep our code predictable, manageable and ultimately bug free.

## Component reuse

The EditProfile and CreateProfile have been reduced to one component [ProfileForm.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/components/profile-forms/ProfileForm.js)  
...
**You can see the implementation of the interceptor and axios instance in [utils/api.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/utils/api.js)**

Creating an instance of axios also cleans up our action creators in [actions/auth.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/actions/auth.js), [actions/profile.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/actions/profile.js) and [actions/post.js](https://github.com/bradtraversy/xdode.exchange_2.0/blob/master/client/src/actions/post.js)
